...
def fetch_proxies():
    global PROXY_LIST, PROXY_LAST_FETCH
    now = time.time()
    if PROXY_LIST and now - PROXY_LAST_FETCH < 600:
        return
    try:
        sources = [
        ...
        if proxies:
            ...
            PROXY_LAST_FETCH = now
            log(f"Загружено {len(PROXY_LIST)} прокси")
    except Exception as e:
        log(f"Ошибка загрузки прокси: {e}")
